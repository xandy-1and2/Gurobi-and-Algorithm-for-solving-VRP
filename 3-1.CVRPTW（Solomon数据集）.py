# 靡不有初，鲜克有终
# 开发时间：2023/10/8 9:49
'''CVRP with Time Window, there is one depot and a single type of vehicles with the same capacity '''
...
def modelingAndSolve(data,M):
    ...
    # 定义变量：
    # Step1.建立合适的数据结构建立变量的索引，CVRPTW有一个决策变量X,一个时间戳辅助变量S（相当于替代了之前的MTZ辅助变量U）
    X_set = []
    S_set = []
...
if __name__ =="__main__":
    ...
    # 求解
    modelingAndSolve(data, M=10000)  # M应当=max{S_ik-S_jk+t_ij} = max{l_i-e_j+t_ij} (也称为max{b_i-a_j+t_ij})
