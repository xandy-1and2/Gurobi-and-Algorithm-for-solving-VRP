# 靡不有初，鲜克有终
# 开发时间：2023/10/8 9:49
'''Classical VRP without time window, there is one depot and a single type of vehicles with the same capacity '''
'''data type: the partial network of the Jiulonghu campus of SEU from OSM'''
import numpy as np
import pandas as pd
...
def readData(path, customerNum, vehicleNum, capacity):
    ...
    SP_data = pd.read_csv(r'C:\Users\Administrator\Desktop\Gurobi-and-Algorithm-for-solving-VRP\data\JiulonghuSP.txt')
...
def modelingAndSolve(data):
    ...
    # 定义变量：
    # Step1.建立合适的数据结构建立变量的索引，经典的VRP仅有一个决策变量X,一个MTZ辅助变量U
    X_set = []
...
if __name__ =="__main__":
    # 数据集路径
    data_path = r'C:\Users\Administrator\Desktop\Gurobi-and-Algorithm-for-solving-VRP\data\Jiulonghu.txt'  # 这里是节点文件
    customerNum = 25
    vehicleNum = 5
    capacity = 100
    # 读取数据
